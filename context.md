...
- `apply_beam_result(beam, blockers, world_positions, path, nuts=[])` — the shared tail of `_update_beam()`: given the routed `path` (or `[]`), toggles the beam node, flashes blockers, and updates `GameManager` puzzle state (beam_blocked / conductor points / `evaluate_puzzle()`). When the beam routes successfully it also calls `better_nut_hint_blockers()` and blinks any blockers that are holding back a higher-nut route. Callers keep their own divergent path computation; only this tail is shared. References the `GameManager` autoload (the one bit of state coupling in this otherwise-pure helper)
- `better_nut_hint_blockers(blockers, start, target, nuts, clear_path)` → Array — recomputes the best route **ignoring blockers** (`best_beam_path([], …)`); if it threads more nuts than `clear_path`, returns the blockers lying on that ideal route, expanded to their connected groups (the blink hint "you could route through more nuts"). Helper `_path_nut_count()`. Returns `[]` when no blockers/nuts or no higher-nut route is being blocked
  ...
  → Main.spawn_prong(player.get_body_center())
      ...
      place prong under Walls tilemap → _update_beam():
          ...
          → activate or deactivate beam; flash blocking blockers
            (incl. blockers holding back a higher-nut route)
...
| Blink hint when blockers hold back a higher-nut route | BeamUtils.gd `better_nut_hint_blockers()` (via `apply_beam_result()`) |
