...
- `_reset_room()` — locks player → ResetEffect fades in → awaits `peaked` → resets room state (prongs, push blocks, fans, breakable walls, key doors, keys, enemies, dust piles, wind turbines, holes) → awaits `done` → unlocks player
...
- `_is_static_solid(gp, include_holes := true)` — checks `walls_tilemap`, `border_walls_tilemap`, and all game-object groups (doors, key_doors, lightning_blockers, screws, breakable_walls, dust_piles, wind_turbines, enemy_doors, closed exit_points, and unfilled holes) so player collision is correct in play mode
