...
**Not Y-sorted with walls:** `ElectricBeam` (`z_index = 10`), `FloorPanel` (`z_index = -10`), `FloorSwitch`/`Hole`/`LightSource` (`z_index = -10`, floor level), UI sprites, camera, overlays.
...
scenes/
  ...
  objects/
    ...
    LightSource.tscn       — Floor light emitter (floor_switch.png sprite); @export id: String; powered exactly like a Door (registers its id via GameManager.register_door and listens to doors_update — no animation, it just tracks a `_powered` bool), and while powered it emits a LIGHT_RADIUS=112px light circle in a dark room (see Dark Rooms below). NOT solid (player walks over it, like FloorSwitch); z_index=-10 (floor level, NOT in Y_SORT_GROUPS); group "light_sources"; sprite dims to 60% grey while unpowered; reset() on room reset clears power. The darkness overlay queries the "light_sources" group and reads is_powered()/get_light_pos()/LIGHT_RADIUS.
