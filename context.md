...
scenes/
  ...
  AbilityPickup.tscn       — ORPHANED: references deleted scripts/AbilityPickup.gd; superseded by PowerOrb.tscn
  ...
  objects/
    ...
    AbilityPickup.tscn     — ORPHANED: references deleted scripts/AbilityPickup.gd; do not use
  ...
  enemies/
    ...
    BounceEnemy.tscn       — BounceEnemy variant; extends WaterEnemy.gd; 50 HP; takes damage only from active fan airflow (−1 HP/frame, immune to electric beam); tile A* pathfinding (Manhattan heuristic) with 1-tile wall/push-block jumps, constrained to home room; bounces instead of sliding; idle rhythmic squish/squash animation; no wall collision; z_index=64 draws above walls
    SpiderEnemy.tscn       — SpiderEnemy variant; extends WaterEnemy.gd; 10 HP; stands still, rotates toward player within 3 tiles (96px), lunges within 2 tiles (64px); 8s stun when HP reaches 0 (sprite→switch_open2.png, no contact kill); hitbox editable via HitboxArea/HitboxShape child nodes
...
scripts/
  ...
  Door.gd                  — Door open/close logic; @export starts_open: bool — when true the door begins open (invisible, passable) and CLOSES immediately (sprite grows in, is_open=false → solid) when the puzzle activates; when the puzzle deactivates a starts_open door re-opens via DoorBall fired from GameManager.last_activator_pos; force_open() instantly sets is_open=true and hides sprite (used by SaveManager on room-solve and load); for normal doors set_open(false) is ignored when SaveManager.is_room_solved() (permanently open in solved rooms); set_open(true) fires a DoorBall from the player to the door center — door stays solid until ball arrives, then _do_open() runs the shrink-to-hidden animation; _opening flag prevents duplicate transitions; set_open(false) cancels any in-flight open
  ...
  Key.gd                   — Collectible; notifies KeyDoor on pickup, plays "vanish" animation then hides; reset() only restores if a KeyDoor still exists in the same room; has a hidden Sprite2D child (key_file4.png at (16,16)) that is immediately hidden in _ready() — used as a palette icon placeholder; z_index set to -5 in _ready() (overrides tscn z_index=5)
  ...
  Enemy.gd                 — Enemy; walks toward player, blocked by walls/solids, instant beam kill via _handle_beam(), resets player on contact; _eject_from_solid() BFS-finds nearest free tile when inside a solid; @export var enemy_id: String = "" — used by EnemyDoor to track kills; is_dead() → bool public accessor
  ...
  FloorSwitch.gd           — Pressure floor switch; group "floor_switches"; z_index=-10; @export id matches door ids; each frame checks if player body center is inside tile rect or any push_block's grid_pos matches; on state change sets GameManager.last_activator_pos to tile center then calls GameManager.set_floor_switch(id, active); reset() deactivates; draws white rect outline when active
...
Sprites/
  ...
  enemies/
    ...
    spider_small.png       — SpiderEnemy sprite (centered=true at (16,16); rotated via _sprite.rotation=_angle; texture faces right at rotation=0, so PI/2 = facing down)
  ...
  objects/
    ...
    floor_switch.png       — FloorSwitch sprite
...
- `_update_beam()` — sets `GameManager.last_activator_pos = player.get_body_center()`, then checks blockers, sets `GameManager.beam_blocked`, calls `evaluate_puzzle()`, activates/deactivates beam
...
- `_transition_to_room(new_room, auto_unlock: bool = true)` — clears prongs instantly, resets enemies in new room, tweens camera 0.25s; when `auto_unlock=false` the camera-tween callback does not unlock the player (used by `_complete_teleport` so the arrive animation controls unlock instead)
...
- `undo_last_push()` — called on Z key (while not `movement_locked`) when `_last_push != null`; blocked (plays electric_fail) if the player is standing at `from_pos` and `is_blocked(from_pos - dir)` (nowhere to push player out); on success: saves entry to `_undo_push`, calls `block.push_undo(from_pos)`, triggers shake (0.8) and `_update_beam()`; if player's tile equals `from_pos`, calls `player.push_out()` to clear the block's rect before moving it back
- `redo_last_push()` — called on Z key when `_last_push == null` and `_undo_push != null`; blocked (plays electric_fail) if the player is standing at the redo destination (`from_pos + dir`); also blocked silently if `can_push_block_to(dest)` is false; on success: restores `_last_push`, calls `block.push(dir)`, triggers shake (0.8) and `_update_beam()`
...
**Groups:** `"fans"`, `"push_blocks"`, `"nuts"` (beam chains through fans when chain ability is active, same as Nut/Screw)
...
**Save data (JSON at `user://save_slot_N.json`):** player world position, current room, abilities dict, push block/nut grid positions (keyed by `start_grid_pos`), collected keys (by `start_grid_pos`), collected power orbs (by pixel position `[x, y]`; on load: marks orbs `_collected=true`, hides them, sets `PowerOrbCounter.count` and emits `count_changed`), opened KeyDoors, open TeleportPanels, removed BossDoors, boss defeated flag, enemy positions + dead flags, map visited rooms. `save_quicksave()` writes the same data structure to `user://quicksave.json`. The shared data-building logic lives in `_build_save_data() -> Dictionary`; both `save()` and `save_quicksave()` call it. `_apply_load` guards `_tween` and `sprite` accesses on push_blocks group members with `get()` to safely skip Fan nodes (which are in `"push_blocks"` but lack these properties).
...
**Modes:** `Mode.BUILD` (palette visible, click to select/inspect objects), `Mode.PLACING` (hint visible, click/drag to place or delete the selected type; Space/Escape returns to BUILD), and `Mode.PLAY` (play-test mode; see Play Mode below). **Wire mode** (`_wire_mode: bool`) is a Boolean overlay on top of BUILD mode that hides the palette and lets the user assign color-coded IDs to ID-capable objects; toggled with E (Escape also exits); Space cycles the active color.

**Palette types (22-column grid):**
`Floor`, `Wall`, `Player`, then all SCENE_MAP keys in order: `PushBlock`, `Nut`, `Screw`, `PassBlock`, `LightningBlocker`, `Door`, `FloorPanel`, `FloorPanelNeg`, `KeyDoor`, `Key`, `FanRight`, `FanLeft`, `FanUp`, `FanDown`, `WindTurbine`, `WindBlock`, `DustPile`, `BreakableWall`, `KeyBreakableWall` (immediately after BreakableWall), `WaterEnemy`, `BounceEnemy`, `SpiderEnemy`.
`SpiderEnemy` palette icon uses `Front_Idle1.png` (editor placeholder).
`KeyBreakableWall` has a composite palette button (Button containing two overlapping TextureRects: wall_breakable.png + Key_File.webp at 50% alpha).
...
- **KeyBreakableWall** — palette-only convenience type; `_place_object("KeyBreakableWall", gp)` calls `_place_object("BreakableWall", gp)` then `_place_object("Key", gp)` in sequence, reusing the Key-on-BreakableWall stacking logic.
...
- **Key on BreakableWall** — only legal stacking: `_place_object` allows Key placement when existing object is BreakableWall. Key gets `modulate.a=0.5` and `z_index=5` **after** `add_child` (Key._ready() resets z_index to -5, so pre-add assignment is overridden).
...
**Drag system:**
- `_drag_placing / _drag_deleting: bool` — set on mouse press, cleared on release
...
**Ghost sprite:** `GhostSprite` (Sprite2D, 45% alpha) follows the mouse in PLACING mode showing the object to be placed. Uses `region_rect` for sprite sheets (first 32×32 frame only). Hidden when Floor is selected.

**Properties panel:** Opens in BUILD mode when clicking a placed object. Shows type label and editable fields for `id`, `id2`, `positive` (via LineEdit / CheckBox widgets).
...
- `get_player_blocking_rects(area)`, `can_push_block_to(gp)`, `get_push_block_at(gp)`, `get_push_block_at_face(...)` — functional stubs querying push_blocks group
- `has_pass_block_at(gp)`, `is_blocked(gp)` — functional stubs (pass_blocks group; static solids + push blocks)
- `_is_static_solid(gp)` — checks `walls_tilemap`, `border_walls_tilemap`, and all game-object groups (doors, key_doors, lightning_blockers, screws, breakable_walls, dust_piles, wind_turbines, enemy_doors) so player collision is correct in play mode
...
- **In play mode:** Space places prongs. Electric beam uses full nut-chaining and blocker logic (same functions as Main.gd). R resets the room (no CRT effect).
- **Exiting play mode (Q):**
