...
- `_update_beam()` — sets `GameManager.last_activator_pos = player.get_body_center()`, computes the beam `path` (via `_compute_beam_path()` when 2 prongs), then delegates to `BeamUtils.apply_beam_result(electric_beam, blockers, world_positions, path)` (the shared body, also used by LevelEditor) which sets `beam_blocked` / conductor points, calls `evaluate_puzzle()`, flashes blockers, and activates/deactivates the beam
